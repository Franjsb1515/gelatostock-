...
- `npm run test:desktop`: **21 PASS con el ejecutable 0.42.0** (salida en reports/desktop-smoke-2026-09-21-v0420.txt). La lanzó el usuario con el equipo despierto, después de dos intentos que se habían cortado en `page.screenshot: Timeout` con la pantalla suspendida. Cubre dentro del ejecutable la comprobación nueva de esta versión —en Mensajes, «Escribir a un proveedor» sin WhatsApp conectado avisa y lleva a la pantalla del canal— y la de la 0.41.0, el «Vale desde» del valor de venta.

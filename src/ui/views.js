...
function settings() {
  return (
    ...
    }</section><section class="panel settings-card"><h2>Plantilla del pedido</h2><p>Es el texto que se envía por WhatsApp. Puedes cambiar saludo, despedida o firma; {lineas} se sustituye por los productos, {numero}, {negocio} y {proveedor} por sus valores.</p><pre class="template-preview">${esc(orderTemplate)}</pre><div class="setting-actions">${btn("Editar plantilla", "orderTemplateEditor", "secondary")}</div></section><section class="panel settings-card"><h2>Plantilla del recordatorio</h2><p>Es el texto que se propone al pulsar «Reclamar respuesta» en un pedido que el proveedor no contesta. Puedes cambiarlo antes de cada envío. {numero} es el número del pedido; {dias}, el tiempo que lleva enviado.</p><pre class="template-preview">${esc(nudgeTemplate)}</pre><div class="setting-actions">${btn("Editar plantilla", "nudgeTemplateEditor", "secondary")}</div></section><section class="panel settings-card"><h2>Respuestas rápidas</h2><p>Los botones que aparecen en un mensaje para responder de un toque. Cambia el texto de los que quieras con tus palabras.</p><p>${Object.keys(replyTemplates).length ? `${Object.keys(replyTemplates).length} con tu texto; el resto, el de origen.` : "Todas con el texto de origen."}</p><div class="setting-actions">${btn("Editar respuestas", "replyTemplatesEditor", "secondary")}</div></section><section class="panel settings-card"><h2>Aviso de mensajes nuevos</h2><p>Cuando un proveedor escribe por WhatsApp, su mensaje entra en Mensajes ya leído por las reglas. Con el aviso activado aparece un recuadro abajo a la derecha con lo que dice, y Windows te avisa aunque la app esté detrás.</p><label class="check-row"><input type="checkbox" id="message-notices" ${messageNotices ? "checked" : ""}> Avisar en cuanto llegue un mensaje</label><p class="fineprint">El aviso no interrumpe lo que estés escribiendo: la pantalla no se redibuja sola. Sin aviso, los mensajes entran igual y los ves al abrir Mensajes.</p></section><section class="panel settings-card"><h2>Identidad del negocio</h2><p>El nombre y el lugar aparecen en la barra lateral, en los mensajes de pedido y en las exportaciones.</p><p><strong>${esc(state.business)}</strong>${state.place ? " · " + esc(state.place) : ""}</p>${btn("Editar identidad", "businessEditor", "secondary")}</section><section class="panel settings-card"><h2>Primeros pasos</h2><ol class="steps"><li><strong>Inventario</strong>: revisa productos, mínimos y presentaciones; registra el stock real con «Registrar stock».</li><li><strong>Proveedores</strong>: completa NIF, WhatsApp con prefijo (+34…) y otros nombres que aparezcan en sus documentos.</li><li><strong>Producción</strong>: crea tus recetas; cada día anota kilos producidos, aprueba el consumo y registra ventas y mermas.</li><li><strong>Compras</strong>: «Preparar reposición», revisa el carrito, autoriza y envía por WhatsApp; registra lo que llega en Control de entregas.</li><li><strong>WhatsApp</strong>: conecta por QR, autoriza los chats de tus proveedores y activa «Mantener la sesión al abrir».</li><li><strong>Mensajes</strong>: mira «Debes leer»; lo demás queda anotado. La IA local es opcional y solo propone.</li><li><strong>Copias</strong>: se hacen solas cada día; restaura desde aquí si hace falta.</li></ol></section><section class="panel settings-card"><h2>Sobre esta versión</h2><p>GelatoStock · versión ${esc(appVersion)}</p><p>Tus datos se guardan en este equipo. Un pedido solo se envía por WhatsApp cuando tú lo confirmas; «Simular envío» prueba el circuito sin escribir a nadie.</p><div class="notice inline">${icon("box")}<span>Esta instalación es independiente. Todavía no sincroniza con otros equipos.</span></div></section></div>`
  );
}
